...
void main()
{
	...
	vec4 data = texture2D( tex, uv_nm, -10.0 );
	vec3 data_yuv = rgb2yuv( data.rgb );
	data_yuv.y /= 1.0-3.0*abs(vt_rnd) * sat( yuv_threshold - vt_rnd );
	data_yuv.z += 0.125 * vt_rnd * sat( vt_rnd - yuv_threshold );
	gl_FragColor = vec4( yuv2rgb(data_yuv), data.a );
}
